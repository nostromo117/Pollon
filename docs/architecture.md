...
### 2. Read Model: PostgreSQL + EF Core (Content Delivery)
Il database di delivery (Content API) utilizza **PostgreSQL** con **Entity Framework Core**.
...
- **Relational Power**: L'uso relazionale di PostgreSQL offre ottime prestazioni per query strutturate e filtraggio dei contenuti pubblicati.
...
### 2. Delivery Search: Unified Search Text (PostgreSQL + EF Core)
...
- **PostgreSQL (Marten)** nel Backoffice gestisce la complessità dei dati semistrutturati (JSONB) e la ricerca avanzata durante la creazione.
- **PostgreSQL (Relazionale)** nella Delivery garantisce stabilità e velocità di risposta per le query di massa orientate alla visualizzazione.
